...
export namespace Coolcar{
    ...
    export async function SendReuqestWithAuthRetry<REQ,RES>(o:RequestOption<REQ, RES>, a?:AuthOption):Promise<RES>{
        const authOpt = a || {
        ...
        try {
            await login()
            return SendReuqest(o, authOpt)
        }catch (err){
            if(err === AUTH_ERR && authOpt.retryOnAuthError ){
                authData.token = ''
                authData.expiryMs = 0
                return SendReuqestWithAuthRetry(o,{
    ...
    export async function login(){
        ...
        const resp = await SendReuqest<auth.v1.ILoginRequest,auth.v1.ILoginResponse>({
            method:'POST',
            path:'/v1/auth/login',
            data:{
                code:wxResp.code,
    ...
    function SendReuqest<REQ,RES>(o:RequestOption<REQ, RES>, a?:AuthOption):Promise<RES>{
            ...
            wx.request({
                ...
                success:res => {
                    if(res.statusCode === 401){
                        ...
                        resolve(
                            o.respMarshaller(camelcaseKeys(res.data as object,{
                                deep:true,
                            }))
                        )
                    }
